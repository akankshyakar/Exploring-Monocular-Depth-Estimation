...
class OrdinalRegressionLoss(object):
    ...
    def __call__(self, log_prob, gt):
        ...
        gt_label = torch.linspace(self.min_depth, self.max_depth, self.ord_num).view(1, self.ord_num, 1, 1).to(gt.device)
        gt_label = torch.log(gt_label)
        gt_label = gt_label.repeat(N, 1, H, W)
        gt_label = gt_label - torch.log(gt + 1e-12)
        gt_label = -gt_label ** 2
